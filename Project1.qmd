...
#     data |>
#       mutate(
#         year = as.numeric(substr(measurement_year, nchar(measurement_year) - 2, nchar(measurement_year) - 1)), 
...
# wrapper_function <- function(data, value_col_name = "enrollment") {
#   
#   data <- read_csv(url)
#   
#   step_1_2()
...
plot_county <- function(data, state = "NC", var_name = "enrollment", top_bottom_county = "top", num_of_county = 5) {
 ...
 # GABBY COMMENT: y = get(var_name)
    ...
    plot <- ggplot(plot_data, aes(x = year, y = .data[[var_name]], color = area_name)) + geom_line() + labs(x = "Year", y = "Mean Value", title = "Line Plot")
