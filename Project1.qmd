...
```{r}
help(separate_wider_position)
```
The enrollment variable contains strings that are 10 digits long. The separate_wider_position() function will help us split up this string and the 7, 2, and 1 numbers are showing where to split things up (first 7 digits are a separate string, next two digits are another string called Year, and then the last digit will be a separate value).

The next line of code creates a new column called Measurement, that has the same values as the Area_name column. 

I think we are trying to create a column called Measurement that has the Area_name values, and the first 7 digits of Enrollment??
...
my_wrapper <- function(url, default_var_name = "default of some kind") {
  select_columns_from_url <- function(url) {
    data <- read_csv(url)
