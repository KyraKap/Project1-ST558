...
#function for steps 1 and 2
  ...
  step_3 <- function(data) {
    data |>
      mutate(year = as.numeric(substr(measurement_year, nchar(measurement_year) - 2, nchar(measurement_year))), measurement = substr(measurement_year, 1, nchar(measurement_year) - 3)
      )
  }
  
  step_5 <- function(data) {
    data |>
      mutate(state = ifelse(grep(pattern = ", \\w\\w", area_name), substr(area_name, nchar(Area_name) - 1, nchar(area_name)), NA))
  }
